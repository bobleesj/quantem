"""Knot geometry + forward warping: everything the knot does to image data.

:class:`DriftKnot` is the centerpiece of the drift-correction architecture.
It owns the K=1 vs K>=2 dispatch and exposes a uniform interface that
:class:`drift.DriftCorrection` reaches for whenever knot mechanics are
needed: per-pixel canvas coordinates (:meth:`to_canvas`), raw-frame drift
...
* :func:`gaussian_smooth_batch` / :func:`gaussian_smooth_1d` — separable
  ...
  knot regularizer in :mod:`drift_optimize`.
...
separate concern and live in :mod:`drift_align`.
"""
...
class DriftKnot:
    ...
    The single dispatch point for K=1 vs K>=2 in the imaging package.
    ...
    the fast axis.  :class:`DriftCorrection` builds one of these per image
    via ``_interpolator(img_idx)`` and routes every knot / canvas / warp
    operation through it so the orchestrator stays focused on the
    algorithm story rather than coordinate math.
    ...
    def affine_candidate_base(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Used by ``_affine_grid_search_batch`` so candidate-broadcast geometry
    ...
    def apply_affine_shift(self, drift_vec: torch.Tensor) -> None:
        ...
        accumulates linearly across the image.  Used by ``align_affine``
    ...
    def warp_to_canvas(
        ...
        lockstep, so ``generate_corrected`` can scatter at a finer grid
...
# Used by :meth:`DriftKnot.warp_to_canvas` (and `_affine_grid_search_batch`'s

...
def integrate_virtual_detector(
    ...
    *,
    ...
    """
    dataset = dataset.array if hasattr(dataset, "array") else dataset
    ...
    if isinstance(dataset, torch.Tensor):
        ...
        image = dataset.reshape(*scan_shape, *detector_shape)[..., mask_t].sum(
            dim=-1,
            dtype=torch.float32,
        )
        if reduce == "mean":
            image = image / num_selected
        return to_numpy(image, dtype=np.float32)
    if hasattr(dataset, "get"):
        dataset = dataset.get()
    array = np.asarray(dataset).reshape(*scan_shape, *detector_shape)
    image = array[..., mask].sum(axis=-1, dtype=np.float64)
    if reduce == "mean":
        image /= num_selected
...
def drift_field(self, idx: int) -> torch.Tensor:
    ...
    corrected = self.interpolator[index].transform_coordinates(self.knots[index])
    nominal = self.interpolator[index].transform_coordinates(self._initial_knots[index])
    return torch.as_tensor(
        np.stack(
            [corrected[0] - nominal[0], corrected[1] - nominal[1]],
            axis=0,
        ),
        dtype=torch.float32,
        device=self._device,
...
def probe_positions(
    ...
    *,
    ...
    knots = self.knots[index] if corrected else self._initial_knots[index]
    row, column = self.interpolator[index].transform_coordinates(knots)
    positions = np.stack([row, column], axis=-1).astype(np.float32, copy=False)
    if strip_padding:
...
def plot_probe_positions(
    ...
    *,
    ...
    """Plot nominal and fitted probe coordinates in shared ``(row, col)`` space."""
    if not isinstance(stride, int) or stride < 1:
        raise ValueError(f"stride must be a positive integer; got {stride!r}.")
    nominal = probe_positions(
        self,
    ...
    axis.quiver(
        nominal[..., 1],
        nominal[..., 0],
        corrected[..., 1] - nominal[..., 1],
        corrected[..., 0] - nominal[..., 0],
        angles="xy",
        scale_units="xy",
        scale=1,
    )
...
def corrected_virtual_images(
    ...
    *,
    ...
    if not hasattr(self, "_initial_knots"):
        raise RuntimeError(
            "corrected_virtual_images() requires preprocess() and correct_affine() first."
        )
    if len(self.images) != 2:
        raise ValueError("corrected_virtual_images() expects exactly two scan images")
    if output_frame not in {"scan", "canvas"}:
        raise ValueError(f"output_frame must be 'scan' or 'canvas'; got {output_frame!r}")
    images = [
        np.asarray(image_0, dtype=np.float32),
        np.asarray(image_1, dtype=np.float32),
    ]
    if images[0].shape != self.images[0].shape or images[1].shape != self.images[1].shape:
        raise ValueError(
    ...
    if output_frame == "canvas":
        ...
        for image_index, image in enumerate(images):
            corrected, coverage = apply_correction_to_dataset(
                self,
                image[..., None],
                image_index=image_index,
                chunk_size=1,
            ...
            components.append(torch.as_tensor(corrected[..., 0], device=self._device))
            coverages.append(torch.as_tensor(coverage, device=self._device))

        valid = [coverage >= 1e-3 for coverage in coverages]
        contribution_count = valid[0].to(torch.float32) + valid[1].to(torch.float32)
        merged = torch.where(
    ...
    for image_index, image in enumerate(images):
        ...
        # Forward projection through the scan-specific interpolator already
        # places both acquisitions in the common image-0 canvas frame.
...
def regional_diffraction_patterns(
    ...
    *,
    ...
    if region_centers.shape != (len(region_names), 2) or not np.isfinite(region_centers).all():
        raise ValueError(
            f"region centers must be finite (row, column) pairs; got shape {region_centers.shape}"
        )
    ...
    if not requested_stages or invalid_stages:
        raise ValueError(
            f"stages must contain 'initial' and/or 'corrected'; got {requested_stages!r}"
        )
    ...
    expected_shapes = [tuple(int(value) for value in image.shape) for image in self.images]
    ...
    if len(detector_shapes[0]) != 2 or detector_shapes[0] != detector_shapes[1]:
        raise ValueError(f"datasets must share one 2-D detector shape, got {detector_shapes}")
...
def diffraction_patterns_at_points(
    self,
    ...
    *,
    ...
    """Select one native diffraction frame nearest each specimen point.

    The scan-space lookup changes between nominal and corrected positions, but
    the selected detector frame is copied directly from the raw acquisition.
    No regional averaging or detector interpolation is performed.
    """
    if not isinstance(points, Mapping) or not points:
        raise ValueError("points must be a non-empty name-to-(row, column) mapping")
    names = tuple(points)
    centers = np.asarray(tuple(points.values()), dtype=np.float32)
    if centers.shape != (len(names), 2) or not np.isfinite(centers).all():
        raise ValueError(
            f"point coordinates must be finite (row, column) pairs; got shape {centers.shape}."
        )
    requested_stages = tuple(stages)
    if not requested_stages or any(
        stage not in {"initial", "corrected"} for stage in requested_stages
    ):
        raise ValueError(
            f"stages must contain 'initial' and/or 'corrected'; got {requested_stages!r}."
        )
    ...
    if source_datasets is None:
        raise RuntimeError(
            "diffraction_patterns_at_points() needs the two raw datasets. "
            "Pass datasets=(scan_0, scan_90) after loading a saved correction."
        )
    source_datasets = tuple(source_datasets)
    if len(source_datasets) != 2 or any(data is None for data in source_datasets):
        raise ValueError("exactly two raw 4D-STEM datasets are required")
    detector_shapes = [tuple(data.shape[2:]) for data in source_datasets]
    if len(detector_shapes[0]) != 2 or detector_shapes[0] != detector_shapes[1]:
        raise ValueError(
            f"datasets must share one two-dimensional detector; got {detector_shapes}."
        )

    selection_shape = (len(requested_stages), len(names), 2)
    patterns = np.empty((*selection_shape, *detector_shapes[0]), dtype=np.float32)
    scan_indices = np.empty((*selection_shape, 2), dtype=np.int32)
    probe_positions_px = np.empty((*selection_shape, 2), dtype=np.float32)
    distances_px = np.empty(selection_shape, dtype=np.float32)
    for stage_index, stage in enumerate(requested_stages):
        corrected = stage == "corrected"
        for scan_index, dataset in enumerate(source_datasets):
            positions = self.probe_positions(
            ...
            for point_index, center in enumerate(centers):
                ...
                row, column = np.unravel_index(flat_index, squared_distance.shape)
                pattern = dataset[row, column]
                patterns[stage_index, point_index, scan_index] = to_numpy(
                    pattern,
                    dtype=np.float32,
                )
                scan_indices[stage_index, point_index, scan_index] = (row, column)
                probe_positions_px[stage_index, point_index, scan_index] = positions[row, column]
                distances_px[stage_index, point_index, scan_index] = np.sqrt(
                    squared_distance[row, column]
                )
    return {
        "patterns": patterns,
        "region_names": names,
        "region_centers_px": centers,
        "selection_mode": "single-probe",
        "stages": requested_stages,
        "scan_direction_degrees": np.asarray(
            self.scan_direction_degrees,
            dtype=np.float32,
...
def corrected_4dstem(
    ...
    *,
    ...
    Each acquisition receives its learned scan-axis field and is projected
    directly into the shared image-0 frame. Preallocated NumPy or memmap
    outputs keep large detector datasets from requiring another full-size
    allocation.
    ...
    if getattr(self, "_datasets_consumed", False):
        raise RuntimeError(
    ...
    if len(datasets) < 2:
        raise ValueError(
            f"Need at least 2 datasets for scan collection correction, got {len(datasets)}"
        )
    ...
    inputs_on_device = (
        isinstance(datasets[0], torch.Tensor)
        and datasets[0].is_cuda
        and isinstance(datasets[1], torch.Tensor)
        and datasets[1].is_cuda
    )
            ...
            for r0 in row_starts:
                ...
                if corrected_4dstem_0.is_floating_point():
                    corrected_4dstem[r0:r1] = (
                        corrected_4dstem_0[r0:r1] + corrected_4dstem_1[r0:r1]
                    ) * 0.5
                else:
    ...
    return CorrectionResult(
        ...
        metadata={
            "scan_axes": (0, 1),
            "detector_axes": (2, 3),
            "detector_shape": tuple(int(value) for value in corrected_4dstem_0.shape[2:]),
            "source_metadata": getattr(self, "_dataset_metadata", None),
        },
    )

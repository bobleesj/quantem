...
if backend == "cuda":
    ...
    torch.cuda.set_per_process_memory_fraction(
        12 * 2**30 / torch.cuda.get_device_properties(0).total_memory
    )
else:
    torch.mps.set_per_process_memory_fraction(12 * 2**30 / torch.mps.recommended_max_memory())
...
def sample():
    while not stop.wait(0.02):
        ...
        stage_peaks[active_stage] = max(stage_peaks.get(active_stage, 0), used)
...
try:
    ...
    result = timed(
        "merge_pack_summaries",
        lambda: model.merge_datasets(
            dtype="scaled_uint16",
            save_to=args.report.parent / "output_master.h5",
            plot_result=False,
            verbose=False,
        ),
    )
    ...
    report = dict(
        ...
        backend=backend,
        timings=timings,
        input_resident_bytes=inputs,
        output_resident_bytes=result.resident_bytes,
        peak=peak,

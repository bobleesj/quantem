...
def test_dataset_container_roundtrip(gold):
    ...
    # ds.virtual_image.adf() defaults to the r..2r band == standalone virtual("ADF")
    np.testing.assert_array_equal(ds.virtual_image.adf(), virtual(gold, "ADF"))
    # second call is served from cache (same array object), not recomputed
    assert ds.virtual_image.bf() is ds.virtual_image.bf()

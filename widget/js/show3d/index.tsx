...
const COLORMAP_NAMES = Object.keys(COLORMAP_POINTS);
...
function formatScaleBarLabel(angstroms: number): string {
  const nice = roundToNiceValue(angstroms);
  if (nice >= 10000) return `${Math.round(nice / 10000)} um`;
  if (nice >= 100) return `${Math.round(nice / 10)} nm`;
  return `${Math.round(nice)} A`;
}
...
function fftshift(data: Float32Array, width: number, height: number) {
  const halfW = width >> 1;
...
// ============================================================================
function Show3D() {
  // Theme detection (matching Show4DSTEM)
  const [themeInfo, setThemeInfo] = React.useState<ThemeInfo>(() => detectTheme());

  // Re-detect theme when body changes (theme switch)
  React.useEffect(() => {
  ...
  // Theme colors based on detected theme (matching Show4DSTEM naming)
  ...
  const [loopStart] = useModelState<number>("loop_start");
  const [loopEnd] = useModelState<number>("loop_end");
    ...
    if (playing) {
      ...
      playIntervalRef.current = window.setInterval(() => {
        setSliceIdx((prev: number) => {
          ...
          if (reverse) {
            if (next < start) return loop ? end : start;
          } else {
            if (next > end) return loop ? start : end;
          }
          return next;
        });
      }, intervalMs);
  ...
  React.useEffect(() => {
    ...
    if (pixelSize > 0 && scaleBarVisible) {
      ...
      drawScaleBar(ctx, canvasW, canvasH, width, pixelSizeAngstrom, themeColors.text, displayScale * zoom, scaleBarLengthPx, scaleBarThicknessPx, scaleBarFontSizePx);
    }
    if (zoom !== 1) {
      ctx.font = "11px sans-serif";
      ctx.fillStyle = themeColors.text;
      ctx.textAlign = "left";
      ctx.fillText(zoom.toFixed(1) + "x", 10, canvasH - 10);
    }
    if (roiActive) {
  ...
  }, [pixelSize, scaleBarVisible, scaleBarLengthPx, scaleBarThicknessPx, scaleBarFontSizePx, roiActive, roiShape, roiX, roiY, roiRadius, roiWidth, roiHeight, isDraggingROI, width, canvasW, canvasH, displayScale, zoom, panX, panY, themeColors]);

  // Render FFT - same size as main image canvas
  React.useEffect(() => {
    ...
    // Store histogram data and stats for FFT
    ...
    ctx.restore();
  }, [showFft, frameBytes, width, height, canvasW, canvasH, fftVminPct, fftVmaxPct, fftColormap, fftLogScale, fftAuto, fftZoom, fftPanX, fftPanY]);
    ...
    <Box className="show3d-root" tabIndex={0} onKeyDown={handleKeyDown} sx={{ ...container.root, bgcolor: themeColors.bg, color: themeColors.text }}>
      <Stack direction="row" spacing={`${SPACING.LG}px`}>
        <Box>
          ...
          {showStats && (
            <Box sx={{ mt: 0.5, px: 1, py: 0.5, bgcolor: themeColors.bgAlt, borderRadius: "2px", display: "flex", gap: 2 }}>
      ...
      {/* Playback controls - bordered row matching Show4DSTEM, constrained to image width */}
      <Box sx={{ ...controlRow, mt: `${SPACING.SM}px`, border: `1px solid ${themeColors.border}`, bgcolor: themeColors.controlBg, width: canvasW, maxWidth: canvasW, boxSizing: "border-box" }}>
        <Stack direction="row" spacing={0} sx={{ flexShrink: 0 }}>
          ...
          <IconButton size="small" onClick={() => { setPlaying(false); setSliceIdx(loopStart); }} sx={{ color: themeColors.textMuted, p: 0.25 }}>
        ...
        <Slider value={sliceIdx} min={0} max={nSlices - 1} onChange={(_, v) => setSliceIdx(v as number)} size="small" sx={{ ...sliderStyles.small, flex: 1, minWidth: 40 }} />

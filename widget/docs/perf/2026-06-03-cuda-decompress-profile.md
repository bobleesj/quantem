...
Data: `/home/owner/ssd/_hf_stage_gold30/gold_30mrad1.3mx04/gold_30mrad1.3mx04_master.h5`
...
| DRAM throughput | 25.6 % | not DRAM-bound |
...
Neither DRAM nor occupancy is the wall. The work is the inherently serial LZ4 token
...
Bitshuffle is **L1/shared-memory-throughput bound (84 %)**, not DRAM-bound (42 %).
...
- **`shuf_8192_16_batched` is the harder floor.** Memory throughput is **83 %** and L1/TEX
  ...
  **barrier stall (49 % of issue cycles)** caused by only 32/256 threads doing the smem load
  while the rest wait. DRAM is only 42 %, so the kernel is **not** moving bytes efficiently
...
- **Mechanism:** collapse `blockIdx.y` (the 16 groups) into one CTA that loads the whole 8 KB
  ...
  256 threads participate in the load → no idle-at-barrier). Expected to move DRAM toward the
  copy-bound roofline (we measured 734 GB/s plain copy bandwidth vs current 42 % DRAM).
- **Plausible gain:** 1.5–2× on the bitshuffle half (it is the more roofline-limited kernel, so
  the gain caps near the L1→DRAM rebalance).
...
- **Hypothesis:** today LZ4 writes the full 19.3 GB unshuffled stream to `lz4_scratch` in DRAM,
  then bitshuffle reads all 19.3 GB back and writes 19.3 GB to `shuf_scratch` — i.e. one extra
  full **write + read of 19.3 GB through DRAM** (~38 GB of traffic) purely to hand off between
...
- **Mechanism:** have the LZ4 kernel decompress an 8 KB block into **smem** (it already streams
  ...
  final reconstructed uint16 to DRAM — never materializing the intermediate unshuffled bytes in
  global memory. Removes one full DRAM write + read of the dataset.

...
class Show3DVolume(anywidget.AnyWidget):
    ...
    # ptycho with nz=14, nxy=730 → set z_stretch=16 to make depth panels readable).
    ...
    compact = traitlets.Bool(False).tag(sync=True)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Auto-pick z_stretch + compact for thin-Z volumes (e.g. multislice ptycho nz=14, nxy=730).
        # User-supplied values always win.
        thin_z_ratio = min(self.nx, self.ny) / max(self.nz, 1)
        if z_stretch is None:
            # Round to half-step matching slider; clamp to validator range [1, 16].
            z_stretch = max(1.0, min(16.0, round(thin_z_ratio * 2) / 2)) if thin_z_ratio > 4 else 1.0
        if compact is None:
            compact = thin_z_ratio > 4
    ...
    def load_state_dict(self, state: dict) -> None:
        ...
        allowed = {
            ...
            "smooth", "flip", "pixel_size", "pixel_size_axes",
